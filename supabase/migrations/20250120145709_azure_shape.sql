/*
  # Fix user metadata and login functionality

  1. Changes
    - Drop and recreate user_metadata table with correct structure
...
BEGIN
    FOR u IN SELECT * FROM auth.users LOOP
        INSERT INTO public.user_metadata (id, email, user_type, is_admin)
        VALUES (
            u.id,
            u.email,
            COALESCE(u.raw_user_meta_data->>'user_type', 'user'),
            CASE WHEN u.email = '1551kkd@gmail.com' THEN true ELSE false END
        )
