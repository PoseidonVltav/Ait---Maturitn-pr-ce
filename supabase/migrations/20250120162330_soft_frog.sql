/*
  # Add Review System for Business Timeout

  1. Changes
    - Add review request columns
    - Add functions for deadline calculation
    - Update policies for review system

  2. Security
    - Only business owners can request review
    - Only admins can approve reviews
*/

-- Add columns for review system
...
-- Policy for business owners to request review
CREATE POLICY "Business owners can request review"
    ON business_profiles
    FOR UPDATE
    TO authenticated
    USING (
    ...
    WITH CHECK (
        ...
        AND (
            review_requested IS DISTINCT FROM review_requested
            OR review_requested_at IS DISTINCT FROM review_requested_at
        )
    );
