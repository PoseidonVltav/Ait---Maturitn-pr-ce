...
export default function BusinessPreviewPage() {
  ...
  const handleTimeout = async () => {
    ...
    try {
      const timeoutDuration = 24;
      const timeoutUntil = new Date();
      timeoutUntil.setHours(timeoutUntil.getHours() + timeoutDuration);
      ...
      setError('Nepodařilo se nastavit timeout');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {isAdmin && (
            <>
              <button
                ...
                title="Pozastavit podnik na 24 hodin"
              >
                <Clock size={20} />
                Timeout
              </button>
      ...
      {business.is_timeout && business.timeout_until && new Date(business.timeout_until) > new Date() && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
          <div className="flex items-center">
            <Clock className="text-yellow-400 mr-2" size={20} />
            <div>
              <p className="text-sm text-yellow-700">
                Tento podnik je dočasně pozastaven do {new Date(business.timeout_until).toLocaleString()}
              </p>
